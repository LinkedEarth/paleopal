...
const MetadataViewer = ({ metadata }) => {
  ...
  const renderValue = (value, key = '') => {
    ...
    if (typeof value === 'boolean') {
      return <span className={`text-xs px-2 py-1 rounded ${value ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
        {value.toString()}
      </span>;
    }
      ...
      return (
        <div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-600">Array ({value.length} items)</span>
            <button
              onClick={() => toggleSection(key)}
          ...
          {isExpanded && (
            <div className="mt-2 space-y-1 max-h-40 overflow-y-auto border-l-2 border-gray-200 pl-3">
              {value.map((item, index) => (
                <div key={index} className="text-xs">
                  <span className="text-gray-500">[{index}]</span> {renderValue(item, `${key}_${index}`)}
                </div>
              ))}
            </div>
          )}
    ...
    if (typeof value === 'object') {
      ...
      return (
        <div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-600">Object ({keys.length} properties)</span>
            <button
              onClick={() => toggleSection(key)}
              className="text-xs text-blue-600 hover:text-blue-800"
            >
          ...
          {isExpanded && (
            <div className="mt-2 space-y-2 max-h-60 overflow-y-auto border-l-2 border-gray-200 pl-3">
              {keys.map((subKey) => (
                <div key={subKey}>
                  <dt className="text-xs font-medium text-gray-600">{subKey}</dt>
      ...
      <div className="space-y-3">
        {Object.entries(metadata).map(([key, value]) => (
          <div key={key} className="border-b border-gray-200 pb-2 last:border-b-0">
            <dt className="text-xs font-medium text-gray-600 uppercase mb-1">{key.replace(/_/g, ' ')}</dt>
...
const AdditionalFieldsViewer = ({ document }) => {
    ...
    if (typeof value === 'object') {
      ...
      return (
        <div>
          ...
          {isExpanded && (
            <div className="mt-2 bg-gray-100 p-2 rounded text-xs font-mono max-h-40 overflow-y-auto">
  ...
  if (fieldsToShow.length === 0) {
    return <div className="text-sm text-gray-500 italic">No additional fields to display</div>;
  }
      ...
      {showRawJSON ? (
        ...
        <div className="space-y-3">
          ...
          <button
            type="button"
            onClick={addField}
            className="w-full px-3 py-2 text-sm border-2 border-dashed border-gray-300 rounded-md text-gray-600 hover:border-gray-400 hover:text-gray-700 transition-colors"
          >
            + Add Metadata Field
          </button>
      ...
      <div className="bg-white shadow-sm border-b px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Libraries Dashboard</h1>
              <p className="text-gray-600 mt-1">Browse and search indexed paleoclimate libraries</p>
            </div>
          </div>
          <div className="flex items-center space-x-4">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 bg-white border-r flex flex-col">
          <div className="p-4 border-b">
            <h2 className="font-semibold text-gray-900 mb-3">Libraries</h2>
            <button
              onClick={() => {
              ...
              className="w-full text-left px-3 py-2 rounded-lg text-sm bg-gray-100 hover:bg-gray-200 transition-colors"
            >
              📊 System Overview
            </button>
          </div>
              ...
              <div key={key} className="border-b border-gray-100">
                <button
                  ...
                  className={`w-full text-left p-4 hover:bg-gray-50 transition-colors ${
                    selectedLibrary === key ? 'bg-blue-50 border-r-2 border-blue-500' : ''
                  }`}
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(library.collections || []).map((collection) => (
                      <span key={collection} className="text-xs px-2 py-1 bg-blue-100 text-blue-800 rounded">
          ...
          {!selectedLibrary ? (
              ...
              <div className="max-w-4xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  <div className="bg-white p-6 rounded-lg shadow">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Libraries</h3>
                    <p className="text-3xl font-bold text-blue-600">{systemStatus.total_libraries || 0}</p>
                    <p className="text-sm text-gray-600">Active libraries</p>
                  </div>
                  <div className="bg-white p-6 rounded-lg shadow">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Collections</h3>
                    <p className="text-3xl font-bold text-green-600">{systemStatus.total_collections || 0}</p>
                    <p className="text-sm text-gray-600">Qdrant collections</p>
                  </div>
                  <div className="bg-white p-6 rounded-lg shadow">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Documents</h3>
                    <p className="text-3xl font-bold text-purple-600">{(systemStatus.total_documents || 0).toLocaleString()}</p>
                ...
                <div className="bg-white rounded-lg shadow overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900">Library Details</h3>
                  </div>
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="bg-white border-b px-6 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-xl font-bold text-gray-900">{libraryDetails?.library.name}</h2>
                    <p className="text-gray-600">{libraryDetails?.library.description}</p>
                  </div>
                  <div className="flex items-center space-x-4">
                ...
                {activeTab === 'overview' && libraryDetails && (
                  <div className="max-w-4xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                      <div className="bg-white p-6 rounded-lg shadow">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">Library Info</h3>
                        <dl className="space-y-2">
                          <div>
                            <dt className="text-sm font-medium text-gray-600">Type</dt>
                            <dd className="text-sm text-gray-900">{libraryDetails.library.type?.replace(/_/g, ' ') || 'unknown'}</dd>
                          </div>
                          <div>
                            <dt className="text-sm font-medium text-gray-600">Collections</dt>
                            <dd className="text-sm text-gray-900">{libraryDetails.library.collections?.length || 0}</dd>
                          </div>
                          <div>
                            <dt className="text-sm font-medium text-gray-600">Total Documents</dt>
                            <dd className="text-sm text-gray-900">{(libraryDetails?.library.total_documents || 0).toLocaleString()}</dd>
                          </div>
                        </dl>
                      </div>
                      <div className="bg-white p-6 rounded-lg shadow">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">Available Filters</h3>
                          ...
                          {(libraryDetails?.collections || []).map((collection) => (
                            <div key={collection.name} className="border border-gray-200 rounded-lg p-4">
                              ...
                              {(collection.sample_documents || []).length > 0 && (
                                <div className="mt-3">
                                  <p className="text-sm font-medium text-gray-700 mb-2">Sample Documents:</p>
                      ...
                      {(libraryDetails.collections || []).map((collection) => (
                        <div key={collection.name} className="bg-white rounded-lg shadow">
                          ...
                          <div className="p-6">
                            {(collection.sample_documents || []).length > 0 ? (
                              <div>
                                <h4 className="font-medium text-gray-900 mb-3">Sample Documents</h4>
                                <div className="space-y-3">
                              ...
                              <p className="text-gray-500">No sample documents available</p>
                            )}
                          </div>
                        </div>
                      ))}
                ...
                {activeTab === 'search' && (
                  <div className="max-w-6xl">
                    {searchResults ? (
                                ...
                                <div key={idx} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                                  ...
                                  {result.method_structure && (
                                    <div className="mt-3 pt-3 border-t border-gray-100">
                                      <h5 className="text-xs font-medium text-gray-700 mb-2">Method Structure</h5>
                                  ...
                                  {result.workflow_steps && (
                                    <div className="mt-3 pt-3 border-t border-gray-100">
                                      <h5 className="text-xs font-medium text-gray-700 mb-2">Workflow Structure</h5>
                                      <div className="text-xs text-gray-600">
                      ...
                      <div className="bg-white rounded-lg shadow p-6 text-center">
                        <div className="space-y-4">
                          <p className="text-gray-500">Enter a search query to find documents in this library</p>
                          {libraryDetails?.collections && libraryDetails.collections.length > 0 && (
                            <div className="text-sm text-gray-400">
                              <p className="mb-2">Available collections:</p>
                              <div className="flex flex-wrap justify-center gap-2">
                                {libraryDetails.collections.map((collection) => (
                                  <span key={collection.name} className="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs">
                                    {collection.name} ({(collection.documents_count || 0).toLocaleString()})
                                  </span>
                                ))}
                              </div>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
                  ...
                  <div className="xl:col-span-1">
                    <h4 className="text-md font-semibold text-gray-900 mb-3">Source Information</h4>
                    <div className="bg-gray-50 p-4 rounded border space-y-3">
                      <div>
                        <dt className="text-xs font-medium text-gray-600 uppercase">Document ID</dt>
                        <dd className="text-sm text-gray-900 font-mono break-all">{documentDetails.id}</dd>
                      </div>
                      <div>
                        <dt className="text-xs font-medium text-gray-600 uppercase">Collection</dt>
                        <dd className="text-sm text-gray-900">{documentDetails.collection}</dd>
                      </div>
                      {documentDetails.source_info.source_file && (
                        <div>
                          <dt className="text-xs font-medium text-gray-600 uppercase">Source File</dt>
                          <dd className="text-sm text-gray-900 break-all">{documentDetails.source_info.source_file}</dd>
                        </div>
                      )}
                      {documentDetails.source_info.content_type && (
                        <div>
                          <dt className="text-xs font-medium text-gray-600 uppercase">Content Type</dt>
                          <dd className="text-sm text-gray-900">{documentDetails.source_info.content_type}</dd>
                        </div>
                      )}
                      {documentDetails.source_info.indexed_at && (
                        <div>
                          <dt className="text-xs font-medium text-gray-600 uppercase">Indexed At</dt>
                          <dd className="text-sm text-gray-900">{new Date(documentDetails.source_info.indexed_at).toLocaleString()}</dd>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="xl:col-span-3">
                    <h4 className="text-md font-semibold text-gray-900 mb-3">Full Document Content</h4>
                    ...
                    {documentDetails.document.content_type === 'complete_method' && documentDetails.document.method_structure ? (
                      <div className="space-y-4">
                        ...
                        {/* Method Steps */}
                        <div>
                          <h5 className="font-semibold text-gray-900 mb-3">Method Steps</h5>
                          <div className="space-y-3">
                            {(documentDetails.document.method_structure.steps || []).map((step, idx) => (
                              <div key={idx} className="bg-gray-50 p-4 rounded border">
                                ...
                                {step.inputs && step.inputs.length > 0 && (
                                  <div className="mb-2">
                                    <span className="text-xs font-medium text-gray-600">Inputs: </span>
                                    <span className="text-xs text-gray-600">{step.inputs.join(', ')}</span>
                                  </div>
                                )}
                                {step.outputs && step.outputs.length > 0 && (
                                  <div className="mb-2">
                                    <span className="text-xs font-medium text-gray-600">Outputs: </span>
                                    <span className="text-xs text-gray-600">{step.outputs.join(', ')}</span>
                                  </div>
                                )}
                                {step.keywords && step.keywords.length > 0 && (
                                  <div>
                                    <span className="text-xs font-medium text-gray-600">Keywords: </span>
                                    <span className="text-xs text-gray-600">{step.keywords.join(', ')}</span>
                                  </div>
                                )}
                              </div>
                            ))}
                      ...
                      <div className="space-y-4">
                        ...
                        {/* Workflow Steps */}
                        <div>
                          <h5 className="font-semibold text-gray-900 mb-3">Workflow Steps</h5>
                          <div className="space-y-3">
                            {(documentDetails.document.workflow_steps || []).map((step, idx) => (
                              <div key={idx} className="bg-gray-50 p-4 rounded border">
                                ...
                                {step.defined_names && step.defined_names.length > 0 && (
                                  <div className="mt-2">
                                    <span className="text-xs font-medium text-gray-600">Defines: </span>
                                    <span className="text-xs text-gray-600">{step.defined_names.join(', ')}</span>
                                  </div>
                                )}
                                {step.used_names && step.used_names.length > 0 && (
                                  <div className="mt-2">
                                    <span className="text-xs font-medium text-gray-600">Uses: </span>
                                    <span className="text-xs text-gray-600">{step.used_names.join(', ')}</span>
                                  </div>
                                )}
                              </div>
                            ))}
                    ...
                    {documentDetails.document.content_type !== 'complete_method' && 
                     documentDetails.document.content_type !== 'complete_workflow' && (
                      <div className="mt-4">
                        <h5 className="text-sm font-semibold text-gray-900 mb-3">Additional Document Fields</h5>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-1">
                    <h4 className="text-md font-semibold text-gray-900 mb-4">Document Settings</h4>
                    ...
                    <div className="space-y-4">
                      ...
                      {/* Document Info */}
                      <div className="bg-blue-50 p-4 rounded border">
                        <h5 className="text-sm font-semibold text-blue-900 mb-2">Document Information</h5>
                      ...
                      <div className="bg-yellow-50 p-4 rounded border">
                        <h5 className="text-sm font-semibold text-yellow-900 mb-2">💡 Tips</h5>
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between flex-shrink-0">
              <div className="text-sm text-gray-500">
                {!newDocumentText.trim() && (
                  <span className="text-red-500">⚠ Document text is required</span>
                )}
              </div>
              <div className="flex items-center space-x-3">
                <button
                  ...
                  disabled={loading}
                  className="px-4 py-2 text-sm bg-gray-300 text-gray-700 rounded hover:bg-gray-400 disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-6 py-2 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
                >
                  {loading ? (
                    <>
