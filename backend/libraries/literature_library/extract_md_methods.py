...
"""literature_library.extract_methods

Extract methods sections from markdown papers using LLM and save to individual JSON files.
...
LOG = logging.getLogger("lit-extract-methods")
...
def filter_methods_sections(sections: List[Dict[str, str]], min_confidence: float = 0.3) -> List[Dict[str, str]]:
    ...
    for section in sections:
        ...
        if is_likely and confidence >= min_confidence:
            ...
            LOG.info(f"Selected section '{section['heading']}' (confidence: {confidence:.2f})")
...
def extract_structured_methods(paper_text: str, engine: str, max_chars: int = 12000) -> Optional[Dict[str, Any]]:
    ...
    LOG.info("Parsing paper into sections...")
    sections = parse_markdown_sections(paper_text)
    LOG.info(f"Found {len(sections)} sections")
    
    # Filter for likely methods sections
    methods_sections = filter_methods_sections(sections, min_confidence=0.3)
...
def get_methods_file_path(md_path: pathlib.Path, output_dir: pathlib.Path = None) -> pathlib.Path:
    """Get the path where methods JSON should be saved for a given markdown file."""
    if output_dir is None:
        output_dir = md_path.parent / "extracted_methods"
    
    # Create output directory if it doesn't exist
    output_dir.mkdir(parents=True, exist_ok=True)
    
    return output_dir / f"{md_path.stem}_methods.json"
...
def process_markdown_files(
    ...
    """
    Process markdown files and extract structured methods using LLM.
    
    Args:
        md_paths: List of markdown file paths
        llm_engine: LLM engine to use ("openai" or "ollama")
        force_reprocess: If True, reprocess even if methods file exists
        output_dir: Directory to save method JSON files (default: {input_dir}/extracted_methods)
    ...
    if output_dir is None and md_paths:
        output_dir = md_paths[0].parent / "extracted_methods"
    
    LOG.info(f"Saving method files to: {output_dir}")
    
    for md_path in md_paths:
        methods_path = get_methods_file_path(md_path, output_dir)
        
        # Skip if methods file already exists (unless force_reprocess)
        if methods_path.exists() and not force_reprocess:
            LOG.info(f"Skipping {md_path.name} - methods file already exists")
        ...
        try:
            ...
            methods_data = extract_structured_methods(paper_text, llm_engine)
            
            if methods_data is None:
                LOG.error(f"Failed to extract methods from {md_path.name}")
                stats["failed"] += 1
                continue
            ...
            # Add metadata
            methods_data["source_file"] = str(md_path)
            ...
            if not methods_data.get("methods_found", False) or not methods_data.get("methods"):
                LOG.warning(f"No methods section found in {md_path.name}")
                stats["no_methods"] += 1
            else:
                ...
                stats["processed"] += 1
                LOG.info(f"Extracted {total_methods} methods with {total_steps} total steps from {md_path.name}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Extract structured methods from markdown papers using LLM")
    ...
    LOG.info(f"Found {len(md_paths)} markdown files")
    
    # Process files
    stats = process_markdown_files(md_paths, llm_engine=args.llm, force_reprocess=args.force, output_dir=output_dir)
